...
class ConnectFourBoard(object):
    ...
    def do_move(self, column):
        ...
        return ConnectFourBoard(new_board, board_already_won=self.is_win(), modified_column=column, current_player = self.get_other_player_id(), k = self._k)
    ...
    def _is_win_from_cell(self, row, col):
        ...
        # generalise to connect-k by comment out the line below
        #return ( self._max_length_from_cell(row, col) >= 4 )
        return ( self._max_length_from_cell(row, col) >= self._k )
    ...
    def clone(self):
        ...
        return ConnectFourBoard(self._board_array, board_already_won=self._is_win, current_player = self.get_current_player_id(), k = self._k)
...
class LongestStreakBoard(ConnectFourBoard):
    ntokens = {1: 10, 2: 10}

    k = sys.maxint

    def __init__(self, board_array = None, board_already_won = None, modified_column = None, current_player = 1, previous_move = -1):
        super(LongestStreakBoard, self).__init__(k = sys.maxint)
    
    def clone(self):
        """ Return a duplicate of this board object """
        return LongestStreakBoard(self._board_array, board_already_won=self._is_win, current_player = self.get_current_player_id())

    def is_win(self):
        # return the id of the winning player, or 0 if none
        if self.ntokens[1] != 0 or self.ntokens[2] != 0:
            # game not yet over
            return 0

        c1 = self.longest_chain(1)
        c2 = self.longest_chain(2)
        if c1 > c2:
            return 1
        elif c2 > c1 :
            return 2
    ...
    def is_tie(self):
        # tie if all players have used their tokens and
        # their longest chains are of equal length 
        if self.ntokens[1] == 0 and self.ntokens[2] == 0 and \
           self.longest_chain(1) == self.longest_chain(2):
               return True
        return False
    ...
    def do_move(self, column):
        ...
        if self.ntokens[player_id] == 0:
            raise InvalidMoveException(column, self)
        
        if self.get_height_of_column(column) < 0:
            raise InvalidMoveException(column, self)

        self.ntokens[player_id] -= 1
        print self.ntokens
        ...
        return LongestStreakBoard(new_board, board_already_won=self.is_win(), modified_column=column, current_player = self.get_other_player_id())
